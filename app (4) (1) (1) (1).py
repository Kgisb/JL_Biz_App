...
if view == "MIS":
    ...
    with st.expander("Settings", expanded=True):
        ...
        src_mode = st.radio("Use JetLearn Deal Source?", ["Yes", "No (All)"], index=0, horizontal=True)
    src_for_pred = source_col if (src_mode == "Yes" and source_col in df_f.columns) else None
    tbl, totals = predict_running_month(df_f, create_col, pay_col, src_for_pred, lookback, weighted, today)
    st.dataframe(tbl, use_container_width=True)
    st.altair_chart(predict_chart_stacked(tbl), use_container_width=True)

    st.markdown("### Backtest")
    back_k = st.slider("Backtest months", 1, 18, 6, 1)
    bt, metrics = backtest_accuracy(df_f, create_col, pay_col, src_for_pred, lookback, weighted, back_k, today)
    st.dataframe(bt, use_container_width=True)
    st.markdown(f"**MAE:** {metrics['MAE']:.2f} • **RMSE:** {metrics['RMSE']:.2f} • **WAPE:** {metrics['WAPE']:.2%} • **MAPE:** {metrics['MAPE']:.2%} • **R²:** {metrics['R2']:.3f}")
    st.altair_chart(accuracy_scatter(bt), use_container_width=True)

elif view == "AC Wise Detail":
    st.subheader("AC Wise Detail")
    st.info("This section is not modified in this request. (Add your existing AC-wise code here.)")

elif view == "Trend & Analysis":
    st.subheader("Trend & Analysis")
    st.info("This section is not modified in this request. (Add your existing Trend & Analysis code here.)")

elif view == "80-20":
    st.subheader("80-20 (Pareto & Mix)")
    group_choice = st.selectbox("Group by", ["JetLearn Deal Source", "Country", "Academic Counsellor"], index=0)
    group_col_map = {
        "JetLearn Deal Source": source_col,
        "Country": country_col,
        "Academic Counsellor": counsellor_col
    }
    gcol = group_col_map[group_choice]
    pareto_tbl = build_pareto(df_f, gcol, group_choice)
    st.dataframe(pareto_tbl, use_container_width=True)
    st.altair_chart(pareto_chart(pareto_tbl, group_choice, f"Pareto: {group_choice}"), use_container_width=True)

elif view == "Stuck deals":
    st.subheader("Stuck deals")
    st.info("This section is not modified in this request. (Add your existing Stuck deals code here.)")

elif view == "Daily business":
    st.subheader("Daily business")
    st.info("This section is not modified in this request. (Add your existing Daily business code here.)")
    ...
    st.info("Welcome! Pick a section from the sidebar to get started.")
